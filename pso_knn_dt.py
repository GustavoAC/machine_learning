...
def fitness(ind):
    ...
    final_selection = [np.abs(base_corr[i]) for i in selected_columns]
    
    if (final_selection == []): return 1
...
print "PSO best individual:", normalized, "with score:", fopt
...
raw_data = filter_set(raw_data, normalized)
test_data = filter_set(test_data, normalized)
